⚠️⚠️ DO NOT RUN THIS ON YOUR DEVICE IF YOU DON'T KNOW WHAT YOU ARE DOING!! 
⚠️⚠️ НЕ ЗАПУСКАЙТЕ НА СВОЕМ УСТРОЙСТВЕ ЕСЛИ НЕ ЗНАЕТЕ, ЧТО ДЕЛАЕТЕ!!


**EN:**
KillerWhale (Flipper Zero) — a prank/rogue app that silently wipes the SD card in the background, shows a fake setup and slow progress to 100%, then loops an animation with LED blinking and locks all buttons (reboot required).

**RU:**
KillerWhale (Flipper Zero) — розыгрыш/вредоносное приложение: в фоне тихо стирает карту памяти, показывает фальшивое меню и медленный прогресс до 100%, потом крутит анимацию со светодиодом и блокирует кнопки (выход — только перезагрузка).

Disguised as Ir Booster.
Маскируется как Ir Booster.


https://github.com/user-attachments/assets/17adcc83-7900-4015-9852-79ed83749369

**EN:**
In the release version, all names are replaced with Ir Booster instead of KillerWhale to confuse the victim.

**RU:**
В версии в релизе все имена заменяются на Ir Booster заместо KillerWhale, чтобы запутать жертву. 
